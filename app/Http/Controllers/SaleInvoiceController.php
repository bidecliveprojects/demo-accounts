...
class SaleInvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $saleInvoices = DB::table('purchase_sale_invoices as psi')
                ->select('psi.*', 'jv.jv_no', 'c.name as customer_name')
                ->join('journal_vouchers as jv', 'psi.jv_id', '=', 'jv.id')
                ->join('customers as c', 'psi.customer_id', '=', 'c.id')
                ...
                ->where('psi.invoice_type',2)
                ->orderByDesc('psi.id')
                ->get();

            return view($this->page . 'indexAjax', compact('saleInvoices'));
        }
    ...
    public function create()
    {
        ...
        $customers = array_filter($customers, fn($s) =>
        ...
        $saleInvoiceSetting = DB::table('invoices_payments_receipts_settings as iprs')
            ...
            ->select('coa.*')
            ->where('iprs.type',2)
            ->where('iprs.option_id',3)
            ->where('iprs.company_id',$companyId)
            ->where('iprs.company_location_id',$companyLocationId)
            ->get();

        return view($this->page . 'create', compact('customers','saleInvoiceSetting'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $journalVoucher->jv_date             = $data['si_date'];
        ...
        // Step 2: Insert Sale Invoice
        $purchaseSaleInvoice = new PurchaseSaleInvoice();
        $purchaseSaleInvoice->invoice_type = 2;
        $purchaseSaleInvoice->invoice_date = $data['si_date'];
        $purchaseSaleInvoice->invoice_no = PurchaseSaleInvoice::VoucherNo(2);
        $purchaseSaleInvoice->slip_no = $data['slip_no'] ?? '-';
        $purchaseSaleInvoice->customer_id = $data['customer_id'];
        $purchaseSaleInvoice->credit_account_id = $data['credit_account_id'];
        ...
        $customer = DB::table('customers')->find($data['customer_id']);
        ...
        return redirect()
            ...
            ->with('message', 'Sale Invoice Created Successfully');
    }
    ...
    public function show(Request $request)
    {
        ...
        $saleInvoiceDetail = \DB::table('purchase_sale_invoices as psi')
            ->select(
                'psi.id',
                'psi.invoice_type',
                'psi.invoice_no',
            ...
            ->join('customers as cus', 'psi.customer_id', '=', 'cus.id')
            ->join('journal_vouchers as jv', 'psi.jv_id', '=', 'jv.id')
        ...
        return view($this->page . 'viewSaleInvoiceDetail', compact('saleInvoiceDetail'));
    }

    public function approveSaleInvoiceVoucher(Request $request){
        $companyId = Session::get('company_id');
        $companyLocationId = Session::get('company_location_id');
        $id = $request->input('id');
    ...
    public function reverseSaleInvoiceVoucher(Request $request){
        $companyId = Session::get('company_id');
        $companyLocationId = Session::get('company_location_id');
        $id = $request->input('id');

        $saleInvoiceDetail = DB::table('purchase_sale_invoices')->where('id',$id)->first();
        ...
        DB::table('journal_vouchers')->where('id', $saleInvoiceDetail->jv_id)->update(['jv_status' => 1]);
        DB::table('journal_voucher_data')->where('journal_voucher_id', $saleInvoiceDetail->jv_id)->update(['jv_status' => 1]);
        DB::table('transaction')
            ->where('company_id',$companyId)
            ->where('company_location_id',$companyLocationId)
            ->where('voucher_id',$saleInvoiceDetail->jv_id)
    ...
    public function edit($id)
    {
        ...
        $saleInvoice = PurchaseSaleInvoice::where('id', $id)
            ->where('company_id', $companyId)
            ->where('company_location_id', $companyLocationId)
            ->firstOrFail();
        ...
        // Filter customers based on company + location
        $customers = array_filter($customers, fn($s) =>
            $s['company_id'] == $companyId &&
            $s['company_location_id'] == $companyLocationId
        );
        ...
        return view($this->page . 'edit', compact('saleInvoice', 'customers','saleInvoiceSetting'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()
            ->route('sale-invoice.index')
            ->with('message', 'Sale Invoice Updated Successfully');
    }
