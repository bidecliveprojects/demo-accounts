@php
    ...
    $counterOne = 1;
    $counterTwo = 1;
    $totalItemSummaryAmount = $detailCustomerAndInvoice->amount;
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-condensed">
                ...
                <tbody>
                    @forelse($receiptSummaryList as $rslRow)
                        @php
                            $totalReceiptAmount += $rslRow->amount;
                        @endphp
                        <tr>
                            <td class="text-center">{{$counterTwo++}}</td>
                            <td>{{$rslRow->rv_no}}</td>
                            <td>{{CommonHelper::changeDateFormat($rslRow->rv_date)}}</td>
                            <td>{{$rslRow->account_head}}</td>
                            <td class="text-right">{{number_format($rslRow->amount,0)}}</td>
                        </tr>
                    @empty
                        <tr>
...
<div class="row">
    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
        <input type="hidden" name="credit_account_id" id="credit_account_id" value="{{$detailCustomerAndInvoice->acc_id}}" />
        <label>Credit Account <span class="text-danger">*</span></label>
        <select disabled class="form-control select2">
            <option>{{$detailCustomerAndInvoice->name}}</option>
        </select>
    </div>
    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
        ...
        <select name="debit_account_id" id="debit_account_id" class="form-control voucher-type @error('debit_account_id') border border-danger @enderror select2">
            @if(count($saleInvoiceSetting) != 0)
                @foreach($saleInvoiceSetting as $key => $row)
                    <option value="{{old('debit_account_id') ?? $row->id}}" @if($detailCustomerAndInvoice->acc_id == $row->id) disabled @endif>{{$row->code}} ---- {{$row->name}}</option>
                @endforeach
            @else
