...
@endphp
@foreach($states as $dRow)
<tr>
    <td class="text-center">{{ $loop->index + 1 }}</td>
    <td>{{ $dRow->country->country_name }}</td>
    <td>{{ $dRow->state_name }}</td>
    <td class="text-center">
        <div class="hidden-print">
            <label class="switch">
                @php
                    $toggleUrl =
                    $dRow->status == 1
                            ? route('states.status', $dRow->id)
                            : route('states.active', $dRow->id);
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu">
                @if($dRow->status == 1)
                    <li><a href="{{ route('states.edit', $dRow->id) }}">Edit</a></li>
                @endif
                <li>
                    <a href="#" class="delete-state-record" data-url="{{ route('states.destroy', $dRow->id) }}">
                        Delete permanently
                    </a>
                </li>
            </ul>
