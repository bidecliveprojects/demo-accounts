...
<div class="well_N">
    <div class="boking-wrp dp_sdw">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                {{ CommonHelper::displayPageTitle('Edit Purchase Invoice') }}
            </div>
        ...
        <form method="POST" action="{{ route('purchase-invoice.update', $purchaseInvoice->id) }}">
            @csrf
            @method('POST')
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="date"
                           ...
                           value="{{ old('pi_date', $purchaseInvoice->invoice_date) }}">
                    @error('pi_date')
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('slip_no', $purchaseInvoice->slip_no) }}">
                    @error('slip_no')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                    ...
                    <select id="supplier_id"
                        ...
                        @foreach($suppliers as $sRow)
                            <option value="{{ $sRow['id'] }}" 
                                {{ old('supplier_id', $purchaseInvoice->supplier_id) == $sRow['id'] ? 'selected' : '' }}>
                                {{ $sRow['name'] }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="debit_account_id"
                        ...
                        @else
                            ...
                            @foreach(CommonHelper::get_all_chart_of_account(1) as $row)
                                <option value="{{ $row->id }}" 
                                {{ old('debit_account_id', $purchaseInvoice->debit_account_id) == $row->id ? 'selected' : '' }}>
                                    {{ $row->code }} — {{ $row->name }}
                                </option>
                            @endforeach
                        @endif
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number"
                           ...
                           value="{{ old('amount', $purchaseInvoice->amount) }}">
                    @error('amount')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
