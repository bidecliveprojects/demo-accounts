...
@endphp
@foreach($purchaseInvoices as $key => $row)
    <tr>
        ...
        <td class="text-center">{{number_format($row->amount,0)}}</td>
        ...
        <td class="text-center">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><a onclick="showDetailModelOneParamerter('purchase-invoice/show','<?php echo $row->id;?>','View Purchase Invoice Detail')"><span class="glyphicon glyphicon-eye-open"></span> View</a></li>
                    @if($row->status == 1 && $row->voucher_status == 1)
                        <li><a href="{{ url('purchase-invoice/' . $row->id . '/edit') }}">Edit</a></li>
                    @endif
                </ul>
            </div>
        </td>
