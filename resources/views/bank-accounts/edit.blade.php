        ...
        <div class="boking-wrp dp_sdw">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    {{ CommonHelper::displayPageTitle('Edit Bank Account Detail') }}
                </div>
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 text-right">
                    <a href="{{ route('bank-accounts.index') }}" class="btn btn-success btn-xs">+ View List</a>
                </div>
            </div>
            <div class="row">
                                ...
                                <select name="acc_id" id="acc_id" class="form-control">
                                    @foreach ($chartOfAccountList as $coalRow)
                                        <option value="{{ $coalRow->code }}"
                                            {{ $bankAccount->acc_id == $coalRow->code ? 'selected' : '' }}>
                                            {{ $coalRow->name }}
                                        </option>
                                    @endforeach
                                </select>
