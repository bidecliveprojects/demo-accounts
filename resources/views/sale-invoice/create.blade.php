...
<div class="well_N">
    <div class="boking-wrp dp_sdw">
        ...
        <div class="row mb-3">
            <div class="col-md-6">
                {{ CommonHelper::displayPageTitle('Add New Sale Invoice') }}
            </div>
        ...
        {{-- Sale Invoice Form --}}
        <form method="POST" action="{{ route('sale-invoice.store') }}">
            @csrf
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select id="customer_id"
                        ...
                        @foreach($customers as $cRow)
                            <option value="{{ $cRow['id'] }}" {{ old('customer_id') == $cRow['id'] ? 'selected' : '' }}>
                                {{ $cRow['name'] }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="credit_account_id"
                        ...
                        @if(count($saleInvoiceSetting) != 0)
                            @foreach($saleInvoiceSetting as $row)
                                <option value="{{ $row->id }}" {{ old('credit_account_id') == $row->id ? 'selected' : '' }}>
                                    {{ $row->code }} — {{ $row->name }}
                                </option>
                            @endforeach
                        @else
