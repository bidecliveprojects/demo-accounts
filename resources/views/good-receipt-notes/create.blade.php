        ...
        <div class="boking-wrp dp_sdw">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    {{ CommonHelper::displayPageTitle('Add New Good Receipt Note') }}
                </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <select name="supplier_id" id="supplier_id" class="form-control select2"
                        ...
                        @foreach ($getSupplierList as $gslRow)
                            <option value="{{ $gslRow->id }}">{{ $gslRow->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="lineHeight">&nbsp;</div>
            <div class="loadGoodsReceiptNoteDetailSection mt-3"></div>
        </div>
    </div>
@endsection
...
@section('script')
    <script>
        function loadPurchaseOrderDetailUsingSupplierId() {
            $('.po-selection').select2();

            var supplierId = $('#supplier_id').val();
            if (supplierId === '') {
                alert('Please Select Supplier Detail');
                $('.loadGoodsReceiptNoteDetailSection').html('');
            } else {
                $('.loadGoodsReceiptNoteDetailSection').html(
                    '<div class="row"><div class="col-lg-12 text-center"><div class="loader"></div>'
                );

                $.ajax({
                    url: '{{ url('/good-receipt-notes/getPurchaseOrdersBySupplierId') }}',
                    type: "GET",
                    data: {
